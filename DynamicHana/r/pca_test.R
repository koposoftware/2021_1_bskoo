...
SELECT MC.MEMBER_NO, H.CARD_NO, H.CATEGORY, H.AMOUNT FROM MEMBER_CARD MC, TRANSACTION_HISTORY H WHERE MC.CARD_NO = H.CARD_NO AND  H.HISTORY_DATE BETWEEN TO_CHAR('2021-09-01') AND TO_CHAR('2021-09-31'))
...
query<-"INSERT INTO PCA_COMPONENT(CATEGORY, CLUSTER_NO) VALUES(?, ?)"
dbSendUpdate(conn, query, result_df$CATEGORY, result_df$CLUSTER_NO)
